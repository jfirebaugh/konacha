...
### Client-side Data Fixtures

Konacha comes with no special fixture support. We suggest you store JavaScript
fixture / factory functions in a dedicated `spec/javascripts/fixtures`
directory.

### Server-side Generated Data Fixtures

Using client-side fixtures brings the risk of having your fixture data diverge
from the JSON serialization generated by the server. To avoid this and keep
your tests DRY, Konacha lets you generate JSON data on the server side and
have it available as a global variable in the JavaScript tests.

To do so, add define a `RubyFixtures.fixtures` function in
`spec/javascripts/ruby_fixtures.rb`, e.g. like so
...
On the client side, you can access the fixture data by calling the
`Konacha.getFixtures()` function, like so:

```javascript
before(function() {
  MyDataStore.load(Konacha.getFixtures().blogs);
});
```

Note that Konacha does not translate between snake_case and camelCase. By
convention, we recommend keeping the fixtures hash in snake_case, since any
models that you might serialize will have snake_cased attributes.

Rails will reload `ruby_fixtures.rb` every time you hit reload, and the
`fixtures` function will run anew. For that reason, try to keep it as fast as
possible.
