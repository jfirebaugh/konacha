...
### Running with other specs

If you run Konacha alongside other specs (e.g. RSpec), it's possible assets will be generated by the other specs which conflict with the Konacaha run. To prevent this, you can either run the Konacha specs first or clear out the old assets first:

```
$ bundle exec rake konacha:run spec
$ bundle exec rake spec assets:clean konacha:run
```
